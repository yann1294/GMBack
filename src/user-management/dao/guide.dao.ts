...
export class GuideDAO implements IGuideDAO {
  ...
  async create(guide: Guide): Promise<ResponseObject> {
    ...
    let profilePhotoResponse: FileServiceResponse =
  ...
  async update(guide: Guide): Promise<ResponseObject> {
    ...
    return await this.dataService.updateDoc(
      this.collectionName,
      guide.uid,
      guide.toUpdateObject(),
    );
  }

  async updatePartial(uid: string, patch: object): Promise<ResponseObject> {
    return this.dataService.updateDoc(this.collectionName, uid, patch);
  }
