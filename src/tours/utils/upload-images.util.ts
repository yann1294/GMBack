...
export async function uploadImages(resourceId: string, images: AsyncIterableIterator<MultipartFile>, resource: 'packages' | 'tours'): Promise<ResponseObject> {
    let destination = `${resource}/${resourceId}`;
    let response: FileServiceResponse = await fileService.uploadFiles(images, destination);
