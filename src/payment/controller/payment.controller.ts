...
export class PaymentController {
  private readonly stripe = new Stripe(process.env.STRIPE_SECRET_KEY, {
    ...
    async stripeWebhook(
      ...
      let payment = (await this.paymentService.findByCondition({
        fieldPath: "sessionId",
      ...
      switch(event.type) {
        case "checkout.session.completed":
          ...
          return await this.paymentService.updatePayment(plainToInstance(PaymentVO, { id: payment.id, status: "completed"}));
        case "checkout.session.expired":
          return await this.paymentService.updatePayment(plainToInstance(PaymentVO, { id: payment.id, status: "failed"}));
      }
